...
class LearnPoseKukaEnv(gym.Env):
    def __init__(self, is_render=True, is_good_view=True, is_train=True):
        ...
        self.x_low_obs=-0.42
        self.x_high_obs=0.42
        self.y_low_obs=-0.42
        self.y_high_obs=0.42
        self.z_low_obs=-0.1
        self.z_high_obs=0.6
        ...
        obs_spaces = {
            ...
            'rays': spaces.Box(low=0, high=2, shape=(109,),dtype=np.float64),
        ...
        self.obs_rays = np.zeros(shape=(109,),dtype=np.float64)
        ...
        # whether collision
        self.collided = None
        # name of current processed component
        self.comp = None
        # path to urdf of robot arm
        self.urdf_root_path = os.path.join(BASE, 'kuka16_description/urdf/kr16_with_MRW510_10GH.urdf')
        # link indexes
        self.base_link = 0
    ...
    def reset(self):
        # random choose a component and a welding spot
        self.comp, curr_spot = random_choice_welding_spot()
        # print ('current component: ',self.comp)

        # based the normals of it place the welding spot in workspace
        comp_position = (-curr_spot[0:3]/1000).tolist()
        self.norm1 = curr_spot[3:6]
        self.norm2 = curr_spot[6:9]
        norm = self.norm1 + self.norm2
        comp_position[0] -= norm[0]*0.4
        comp_position[1] -= norm[1]*0.4
        self.target_position = [-norm[0]*0.4, -norm[1]*0.4, 0.0]
        ...
        self.RobotUid = p.loadURDF(self.urdf_root_path, basePosition=[0,-1.5,0.0], baseOrientation=baseorn, useFixedBase=True)
        # print (self.comp)
        
        # load component
        self.object_id=p.loadURDF(PATH_COMP+'/'+self.comp+'/model.urdf',basePosition=comp_position,useFixedBase=True)

        # kuka goes to the initial position
        go_to_target_kuka(self.RobotUid, self.base_link, self.effector_link, [0.0,0.0,0.4], [0,0,0])
        # show welding spot
    ...
    def _reward(self):
        ...
        # check if torch is out of boundary
        terminated=bool(
            x<self.x_low_obs
    ...
    def _set_lidar(self, ray_length=2, ray_num_hor=36, render=False, show_pc=False):
        ray_froms = []
        ray_tops = []
        frame = quaternion_matrix(self.current_orn)
        ...
        # if show_pc:
        #     converted_pc = np.zeros(shape=(len(ray_froms),3))
        #     for i in range(len(ray_froms)):
        #         converted_pc[i,:] = np.asarray(results[i][3])
            
        #     point_cloud2 = PointCloud2()
        #     point_cloud2.header.frame_id = "map"
        #     converted_pc2 = pc2.create_cloud_xyz32(point_cloud2.header, converted_pc)
        #     self.pc_pub.publish(converted_pc2)
...
if __name__ == '__main__':
    ...
    env = LearnPoseKukaEnv(is_render=True, is_good_view=False)
    episodes = 10
