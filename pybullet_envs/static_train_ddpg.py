...
if __name__=='__main__':
     if IS_TRAIN:
          ...
          checkpoint_callback = CheckpointCallback(save_freq=51200, save_path='./models/static_ddpg_ckp_logs/',
                                             name_prefix='static')
          # Create the callback list
          callback = CallbackList([checkpoint_callback, callback_max_episodes])
          n_actions = env.action_space.shape[-1]
          action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=0.1*np.ones(n_actions))
          model = DDPG("MlpPolicy", env, batch_size=128, action_noise=action_noise, verbose=1, tensorboard_log="./models/static_ddpg_tf_logs/")
