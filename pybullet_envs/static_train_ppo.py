...
if __name__=='__main__':
     if IS_TRAIN:
          ...
          eval_callback = EvalCallback(eval_env, best_model_save_path='./models/best_static/',
                             log_path='./models/best_static/', eval_freq=10000,
                             deterministic=True, render=False)
          
          # Save a checkpoint every ? steps
          checkpoint_callback = CheckpointCallback(save_freq=51200, save_path='./models/static_ppo_ckp_logs/',
                                             name_prefix='static')
          # Create the callback list
          callback = CallbackList([checkpoint_callback, callback_max_episodes])
          model = PPO("MlpPolicy", env, batch_size=128, verbose=1, tensorboard_log="./models/static_ppo_tf_logs/")
          # model = PPO.load('./ppo_ckp_logs/reach_?????_steps', env=env)
          model.learn(
               total_timesteps=1e10,
               n_eval_episodes=64,
               callback=callback)
          model.save('./ppo_static')
     else:
          # load env
