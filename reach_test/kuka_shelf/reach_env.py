...
class MySimpleReachEnv(gym.Env):
    def __init__(self, is_render=False, is_good_view=False, is_train=False):
        ...
        self.obs_rays = np.zeros(shape=(25,),dtype=np.float32)
        obs_spaces = {
            'position': spaces.Box(low=-2, high=2, shape=(14,), dtype=np.float32),
            'rays': spaces.Box(low=0, high=2, shape=(25,),dtype=np.float32),
        } 
    ...
    def _set_lidar_cylinder(self, ray_min=0.02, ray_max=0.4, ray_num_ver=1, ray_num_hor=12, render=False):
        ray_froms = []
        ray_tops = []
        frame = quaternion_matrix(self.current_orn)
        frame[0:3,3] = self.current_pos
        ray_froms.append(list(self.current_pos))
        ray_tops.append(np.matmul(np.asarray(frame),np.array([0.0,0.0,0.5,1]).T)[0:3].tolist())
        ...
        for angle in range(230, 240, 10):
            for i in range(ray_num_hor):
                z = -0.5 * math.sin(angle*np.pi/180)
                l = 0.5 * math.cos(angle*np.pi/180)
...
if __name__ == '__main__':   
    ...
    #                             )
    # while True:
    #     go_to_target_kuka(RobotUid, 0,6,[-0.5,0.8,0.5],[])
